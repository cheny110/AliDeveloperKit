...
extern void myLcdWriteReg(uint8_t Data);
extern void myLcdWriteData(uint8_t Data);
extern void myLcdWriteDataMultiple(uint8_t * pData, int NumItems);
...
void ST7789H2_WriteReg(uint8_t Command, uint8_t *Parameters, uint8_t NbParameters)
{
  ...
  /* Send command */
  myLcdWriteReg(Command);
  
  /* Send command's parameters if any */
  for (i=0; i<NbParameters; i++)
  {
    myLcdWriteData(Parameters[i]);
  }
}
...
void BSP_LCD_Clear(uint16_t Color)
{
  ...
  for (counter = 0; counter < 240; counter++)
  {
		...
		myLcdWriteDataMultiple(black_gui, 480);	 
  }
}
...
void ST7789H2_WriteLine(uint16_t Xpos, uint16_t Ypos, uint16_t *RGBCode, uint16_t pointNum)
{
  ...
  /* Write RAM data */
  myLcdWriteDataMultiple(endian_buffer, pointNum*2);
}
